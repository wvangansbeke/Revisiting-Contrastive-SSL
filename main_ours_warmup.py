...
import builder.builder_ours_warmup
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = builder.builder_ours_warmup.MoCo(
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    if epoch >= args.warmup_epochs:
        print('Training with aux loss - Epoch {} - Warmup {}'.format(epoch, args.warmup_epochs))
        use_aux = True
        ...
        print('Training without aux loss - Epoch {} - Warmup {}'.format(epoch, args.warmup_epochs))
        use_aux = False
        progress = ProgressMeter(
            len(train_loader),
            [batch_time, data_time, total_losses, losses, top1, top5],
            prefix="Epoch: [{}]".format(epoch))
